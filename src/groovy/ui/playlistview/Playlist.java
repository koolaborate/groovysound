 ...
 * Playlist                                                                        *
 ***********************************************************************************
 * The playlist containing PlaylistEntries (a list of songs of the current album). *
 ***********************************************************************************
...
public class Playlist extends JPanel
{
	...
	public void showPopup(Component c, int x, int y)
	{
		...
		JMenuItem playItem = new JMenuItem(LocaleMessage.getString("playlist.play_song"));
		...
		JMenuItem infoItem = new JMenuItem(LocaleMessage.getString("playlist.informations"));
		...
		JMenuItem id3Item = new JMenuItem(LocaleMessage.getString("id3.edit_tags"));
...
//				song.add(getCurrentlySelectedSongPath());
				...
				int albumId = window.getDatabase().getAlbumIdForSong(getCurrentlySelectedSongID());
		...
		JMenuItem delItem = new JMenuItem(LocaleMessage.getString("playlist.del_song"));
			...
			public void actionPerformed(ActionEvent arg0)
			{
				...
				if(delDiag.yesSelected)
				{
					...
					if(delDiag.delFilesSelected)
					{
						...
						if(!StringUtils.isNullOrEmpty(path)) FileHelper.removeFile(path);
					}
	 ...
	 * 
	 * @param songs the songs to be filled into the playlist
	 */
	public void setSongs(List<Song> songs)
	{
		...
		for(Song s : songs)
		{
			...
			PlaylistEntry entry = new PlaylistEntry(this, s.getFileName(), s.getDuration(), s.getId(), s.getTitle(), 
					d.getSelectionColor2(), d.getSelectionColor1(), d.getSelectionAlpha());
	...
	public void setPlayIconAtCurrentEntry()
	{
		...
		{
			if(e.isPlaying()) e.setPlaying(false);
	 ...
	 * Sets the album path.
	 * 
	 * @param path the path of the current album
	 */
	...
	public String getCurrentlySelectedSongPath()
	{
		...
		{
			if(entries.size() > 0) selectedEntry = entries.get(0);
			else return null;
		}
	...
	public String getCurrentlySelectedSongID()
	{
		...
		String ret = null;
		if(selectedEntry != null) ret = Integer.toString(selectedEntry.getSongId());
	 ...
	 * 
	 * @return <code>true</code> if there are more songs in the list, <code>false</code> if it was the last song
	 */
	public boolean selectNextSong()
	{
		int selectIndex = entries.indexOf(selectedEntry)+1;
		// TODO if continuous play is selected, continue with first song in the list
		if(entries.size() <= selectIndex) return false;
		else
		{
			// select next row
