...
import ui.playlistview.CoverAndInfoPanel;
import ui.playlistview.PlaylistPanel;
import ui.themes.Theme;
import ui.themes.ThemeHelper;
import ui.tinyview.TinyView;
import ui.trayicon.TrayIconHandler;
import db.Database;
...
{
	// images
	private static BufferedImage logoImg, logoTextImg, minimizeImg, closeImg, helpImg,
		minimizeOverImg, closeOverImg, helpOverImg, maximizeImg, maximizeOverImg, 
		maximize2Img, maximize2OverImg, tinyImg, tinyOverImg;
	...
	public MainWindow(Settings s)
	{
		...
		if(p == null) setLocationRelativeTo(null); // center on screen if no value is given
		else setLocation(p);
	...
	{
		// retrieve the active theme name from the XML file. If the file is not present, 
		String themeName = ThemeHelper.retrieveActiveThemeName();
		
		if(StringUtils.isNullOrEmpty(themeName) || themeName.equals("DefaultTheme"))
		{
			return;
		}
	...
	private JPanel createHeaderPanel()
	{
		...
		tinyButton.addActionListener(new ActionListener(){			
			public void actionPerformed(ActionEvent e)
			{
				...
				tinyWindow = new TinyView(getThisInstance(), 
						db.getAlbumById(songInfo.getAlbumId()), songInfo.getSongID());
		...
		maximize.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent e)
			{
				if(getExtendedState() != MAXIMIZED_BOTH) maximize();
				else deMaximize();
	...
	/**
	 * Method to maximize the window. The window shape has to be made rectangular without 
	 * rounded corners.
	 */
	...
	/**
	 * Sets the window shape using reflection since the AWTUtilities class is not yet 
	 * official and only present in JRE 1.6_10 and newer.  
	 * 
	 * @param roundCorners whether or not to give the window smooth rounded corners
	 */
	private void setWindowShape(boolean roundCorners)
	{
		...
		if(roundCorners)
		{
			roundWindowSupported = winShaper.shapeWindow(this, 
					new RoundRectangle2D.Double(0, 0, getWidth(), getHeight(), 20, 20));
			if(roundWindowSupported && mainPanel != null) 
				mainPanel.setBorder(new RoundedBorder(20));
		}
		else
		{
			winShaper.shapeWindow(this, new Rectangle2D.Double(0, 0, getWidth(), getHeight()));
		}
	}
	
	/**
	 * Method to de-maximize the window. The window shape has to be made rectangular with rounded 
	 * corners again.
	 */
	...
	/**
	 * A class to enable moving the window since the window decorations have been removed.
	 */
	protected class InnerListener extends MouseAdapter implements MouseMotionListener 
	{
		...
		public void mouseClicked(MouseEvent e) 
		{
			if(e.getClickCount() == 2 && 
					e.getButton() == MouseEvent.BUTTON1 && e.getComponent() == header)
			{
				if(getExtendedState() == MAXIMIZED_BOTH) deMaximize();
				else maximize();
			}
		}
		...
		{
			if(e.getButton() == MouseEvent.BUTTON1) startDrag = e.getPoint();
			else startDrag = null;
		}
		...
		public void mouseDragged(MouseEvent e) 
		{
			if(startDrag != null) 
			{
				...
				else 
				{
					...
					if(width < getMinimumSize().width) width = getMinimumSize().width;
					if(height < getMinimumSize().height) height = getMinimumSize().height;
	 ...
	 * 
	 * @param songInfo the current song info to retrieve the artist's name
	 */
...
//		centerPanel.getTimeElapsedPanel().setTotalTimeLabelText(songInfo.getDuration());
		centerPanel.getTimeElapsedPanel().setSongFileSize(new File(songInfo.getSongFilePath()).length()); 
	...
	public void drop(DropTargetDropEvent dtde)
	{
		...
		if(centerPanel.getComponentAt(p) instanceof JPanel)
		{
			...
			if(styledBackground.getName() != null && styledBackground.getName().equals("ACTION"))
			{
	...
	/**
	 * Sets a new cover when it is dropped via drag n drop. The given cover image will be
	 * copied into the destination folder as hidden folder.jpg and afterwards the cover image
	 * will be repainted.
	 * 
	 * @param absolutePath the path to the new cover image
	 */
	...
	/**
	 * Sets a new cover image and replaces the existing one if necessary. Also, a thumbnail view is
	 * generated and the old thumbnail (if available) is deleted.
	 * 
	 * @param newCover the new cover image
	 */
	public void setNewCover(BufferedImage newCover)
	{
		...
		try 
		{
			...
			if(!StringUtils.isNullOrEmpty(destFolder))
			{
				...
				if(oldCover.exists()) oldCover.renameTo(new File(destFolder + 
...
//				repaintBackgroundPanel();
		...
		{
			VistaDialog.showDialog(LocaleMessage.getString("error.1"), LocaleMessage.getString("error.26"), 
	...
	public void setCurrentNavigation(NAVIGATION nav)
	{
		...
		if(nav == NAVIGATION.PLAYLIST) enableDnD = true;
	...
	/**
	 * Registers a global listener to show the help in case the F1 key is pressed.
	 */
        ...
        e.push(new EventQueue(){
            protected void dispatchEvent(AWTEvent event)
            {
                if(event instanceof KeyEvent)
                {
                	...
                	// only at the event key released and only if it is the F1 key
                	if(k.getID() == KeyEvent.KEY_RELEASED && k.getKeyCode() == KeyEvent.VK_F1) showHelp();
                }
	...
	private void initHelp()
	{
		...
		String testlang = Locale.getDefault().getLanguage(); // get the language code for the current locale, for example "de"
		File testFolder = new File(helpdir + testlang); // check if the help is available for this language ("help_de")
		if(testFolder.exists() && testFolder.isDirectory()) lang = testlang; // use it if it is available
	 ...
	 * 
	 * @param coverPath the absolute path to the cover image (will only be changed if it
	 *        differs from the currently shown image)
	 * @param artist the name of the artist
	 * @param title the name of the album or song
	 */
