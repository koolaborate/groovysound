...
import ui.playercontrols.NextButton;
import ui.playercontrols.PlayButton;
import ui.playercontrols.PreviousButton;
import ui.playercontrols.StopButton;
...
public class PlaybackController implements BasicPlayerListener
{
	 ...
	 * Constructor.
	 * 
	 * @param cPanel the reference to the center panel
	 * @param playlist the reference to the song playlist
	...
	public void playSong()
	{
		try
		{
			...
			if(currentState == STATE.PLAYING || currentState == STATE.PAUSED) control.pause();
			...
			if(!f.exists()) 
			{
				// in case the file doesn't exist any more, the user has to be asked whether or not
				// to remove the song from the list. If the whole album cannot be found any more,
...
//			}).start();
			...
			// HOTFIX: adjust the volume once again since it did not work under Vista in 
			// the first place the volume was always at max when a new song was started...
			adjustVolumeAndBalance();
		...
		catch(BasicPlayerException e)
		{
			...
			if(e.getMessage().equals("java.io.IOException: Resetting to invalid mark"))
			{
				// this exception is thrown if the mp3 file contains an embedded cover
				...
				// Instead of reading the file directly, use a FileInputStream and
				...
				try
				{
					...
					try { m.close(); } catch(Exception ex) { ex.printStackTrace(); }
			...
			else
			{
				VistaDialog.showDialog(LocaleMessage.getString("error.1"), 
						...
						LocaleMessage.getString("error.23"), VistaDialog.WARNING_MESSAGE);
	...
	/**
	 * Handles the case if a song file is no longer existent under the given path.
	 */
	private void handleSongNoLongerAvailable() 
	{
		...
		if(!StringUtils.isNullOrEmpty(path))
		{
			...
			if(!folder.exists())
			{
				VistaDialog delAlbum = VistaDialog.showConfirmationDialog(LocaleMessage.getString("error.15"),
						LocaleMessage.getString("error.16"), LocaleMessage.getString("error.17") + " '" +
						" " + path + " ' " + LocaleMessage.getString("error.20") + "\n\n" + LocaleMessage.getString("error.18"));
				if(delAlbum.yesSelected)
				{
					...
					if(albumId != -1) window.getDatabase().deleteAlbum(albumId);
					...
					// refresh the playlist view (load empty cover since it was the active 
			...
			{
				VistaDialog delSong = VistaDialog.showConfirmationDialog(LocaleMessage.getString("error.13"),
						LocaleMessage.getString("error.12"), LocaleMessage.getString("error.19") +
						" '" + filename + "' " + LocaleMessage.getString("error.20") + LocaleMessage.getString("error.21"));
				if(delSong.yesSelected)
				{
					int songId = window.getPlaylist().getCurrentlySelectedSongID();
	 ...
	 * Sets the volume.
	 * 
	 * @param vol the desired volume between 0.0f and 1.0f
	 */
	 ...
	 * 
	 * @param bytes the amount of bytes to which the player shall seek to
	 */
...
//		System.out.println("progress : " + properties.toString());
		...
		centerPanel.getTimeElapsedPanel().updateSlider((int)(long)(Long)properties.get("mp3.position.byte"));
	}
	 ...
	 * 
	 * @param balance a value between -1.0f (left) and 1.0f (right). 0.0f means middle
	 */
	...
	/**
	 * @return the current state (either STATE.PLAYING, STATE.PAUSED or STATE.ENDED)
	 */
