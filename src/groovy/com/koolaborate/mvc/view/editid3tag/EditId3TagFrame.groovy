package com.koolaborate.mvc.view.editid3tag
 ...
 * EditId3TagFrame *
 *********************************************************************************** 
 * A window that enables the used to edit the id3 tags of one or multiple mp3
 * song * files. *
 *********************************************************************************** 
...
class EditId3TagFrame extends JFrame{
	...
	JTextField titleText, artistText, tracknoText, yearText, albumText
	JTextField genreText
	JCheckBox cTitle, cArtist, cTrackno, cYear, cAlbum, cGenre

	boolean multipleSongs = false
	 ...
	 * @param albumId
	 *            the id of the album the song(s) belong to
	 * @param albumPath
	 *            the path to the album folder
	 * @param songPath
	 *            the list of song file paths to be edited (may only contain one
	 *            element)
	 */
	public EditId3TagFrame(MainWindow window, int albumId, String albumPath, List<String> songFileNames){
		this.window = window
	 ...
	 * 
	 * @param c
	 *            the component to be enabled or disabled
	 * @param b
	 *            <code>true</code> if it shall be enabled, <code>false</code>
	 *            otherwise
	 */
	...
	private JPanel createButtonPanel(){
		JPanel p = new JPanel()
		p.setBorder(new VariableLineBorder(5, 5, 5, 5, Color.GRAY, 1, true,
				false, false, false))
	...
	private void loadSongInfos(){
		...
		// also set all checkboxes to unchecked (meaning: don't make any
		// changes)
		...
		if(multipleSongs) {
				...
				if(file.exists()) {
					try {
						...
						if(!currentArtist.equals(different)) {
							...
							if(StringUtils.isEmpty(currentArtist))
								currentArtist = artist
							// if the artists differ, set current artist to
							// 'different values'
						...
						if(!currentAlbum.equals(different)) {
							...
							if(StringUtils.isEmpty(currentAlbum))
								currentAlbum = album
						...
						if(!currentGenre.equals(different)) {
							...
							if(StringUtils.isEmpty(currentGenre))
								currentGenre = genre
			...
			// the track number and the song's title will always be different
			// when there
			// are multiple files...
		...
		// if it is only one song to be edited
			...
			File file = new File(albumPath + File.separator
					+ songFileNames.get(0))
			if(file.exists()) {
				try {
					MP3File f = (MP3File) AudioFileIO.read(file)
					ID3v1Tag v1Tag = f.getID3v1Tag()
					String title = "", artist = "", track = "", year = "", album = "", genre = ""
					...
					if(StringUtils.isEmpty(title))
						title = getFromID3v1Tag(v1Tag.getTitle())
					if(StringUtils.isEmpty(artist))
						artist = getFromID3v1Tag(v1Tag.getArtist())
					// if(StringUtils.isNullOrEmpty(track)) track =
					// getFromID3v1Tag(v1Tag.getTrack());
					if(StringUtils.isEmpty(year))
						year = getFromID3v1Tag(v1Tag.getYear())
					if(StringUtils.isEmpty(album))
						album = getFromID3v1Tag(v1Tag.getAlbum())
					if(StringUtils.isEmpty(genre))
						genre = getFromID3v1Tag(v1Tag.getGenre())
	...
	/**
	 * Method tries to retrieve information from the ID3v1 tag. If the given
	 * list is empty or <code>null</code> an empty String is returned.
	 * 
	 * @param tagList
	 *            the list of elements for the given tag, may be empty or
	 *            <code>null</code>
	...
	private void saveChanges(){
		...
		for(String songFileName: songFileNames) {
			...
			// maybe the song name has been modified to be able to save the name
			// into the database
			...
			if(file.exists()) {
				try {
					...
					String oldTitle = "", oldArtist = "", oldTrack = "", oldYear = "", oldAlbum = "", oldGenre = ""
					...
					if(StringUtils.isEmpty(oldTitle))
						oldTitle = getFromID3v1Tag(v1Tag.getTitle())
					if(StringUtils.isEmpty(oldArtist))
						oldArtist = getFromID3v1Tag(v1Tag.getArtist())
					// if(StringUtils.isNullOrEmpty(oldTrack)) oldTrack =
					// getFromID3v1Tag(v1Tag.getTrack());
					if(StringUtils.isEmpty(oldYear))
						oldYear = getFromID3v1Tag(v1Tag.getYear())
					if(StringUtils.isEmpty(oldAlbum))
						oldAlbum = getFromID3v1Tag(v1Tag.getAlbum())
					if(StringUtils.isEmpty(oldGenre))
						oldGenre = getFromID3v1Tag(v1Tag.getGenre())

					log.debug("Processing file " + file.getAbsolutePath()
							+ "...")

					// if the title shall be changed
					if(cTitle.isSelected() && !StringUtils.isEmpty(titleText.getText().trim())) {
					...
					if(cArtist.isSelected()
							&& !StringUtils.isEmpty(artistText.getText().trim())) {
					...
					if(cTrackno.isSelected()
							&& !StringUtils.isEmpty(tracknoText.getText()
									.trim())) {
						String newTrackno = tracknoText.getText().trim()
						// only make changes if the new track number is any
						// different
					...
					if(cYear.isSelected()
							&& !StringUtils.isEmpty(yearText.getText().trim())) {
					...
					if(cAlbum.isSelected()
							&& !StringUtils.isEmpty(albumText.getText().trim())) {
					...
					if(cGenre.isSelected()
							&& !StringUtils.isEmpty(genreText.getText().trim())) {
					...
					log.error("Unable to set new values for file "
							+ file.getAbsolutePath() + ". Error message: "
							+ e.getMessage())
				}
			}
			// if file does not exist
				...
				log.debug(file.getAbsolutePath()
						+ " does not exist! Unable to set new values.")
			}
		}
